...
# ──────────────────────────────────────────────────────────────────────────────
# FILTER BUILDERS – WITHOUT DATE (to avoid field validation errors)
# ──────────────────────────────────────────────────────────────────────────────
def build_analytics_filter(region, acm, agency_type):
    """
    Build a filter object for the Requests table, excluding date.
    Only includes Region, ACM (via multiple fields), and Agency Type.
    """
    conditions = []
...
def build_points_filter(agency_code, region, acm, search):
    """
    Build a filter for the Points table – all fields are expected to exist.
    """
    conditions = []
    if agency_code:
        conditions.append({
...
# ──────────────────────────────────────────────────────────────────────────────
# REFACTORED ANALYTICS ENDPOINT – Live, filtered (region/acm/type), date in‑memory
# ──────────────────────────────────────────────────────────────────────────────
...
def health():
    return jsonify({
        ...
        "message": "Analytics endpoints use server-side filtering (region/acm/type) + date filtering in-memory."
    })
