...
# --- 6.5 Centralized Configuration & Setup ---
app = Flask(__name__)

# 7.2 Structured Logging Stub (Standardized format)
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - [XenaPortal] - %(message)s'
)
logger = logging.getLogger(__name__)

# 2.1 API Response Caching Layer
cache = Cache(app, config={'CACHE_TYPE': 'SimpleCache', 'CACHE_DEFAULT_TIMEOUT': 300})

# 7.1 Rate Limiting (Protects Feishu API Quota)
limiter = Limiter(
    get_remote_address,
    app=app,
...
ACCESS_TABLE_ID = "tbl3wweYCpmDSx"

# 4.3 Role/Access Defaults
...
# --- 7.3 PII Masking Utility ---
def mask_email(email):
    if not email or '@' not in email: return email
    name, domain = email.split('@', 1)
    if len(name) <= 2: return f"***@{domain}"
    return f"{name[0]}***{name[-1]}@{domain}"

# --- Helper Functions ---
@cache.memoize(timeout=3500) # Cache tenant token for slightly less than 2 hours
...
# --- Core Access Control ---
@cache.memoize(timeout=60) # Memoize access checks for 60s to prevent spamming Feishu
...
def callback():
    ...
    # 4.1 Basic Audit Logging for logins
    logger.info(f"USER_LOGIN: {lark_name} ({mask_email(lark_email)})")
...
def manage_users():
    ...
    if request.method == 'GET':
        ...
        # 4.1 Audit Log
        logger.info(f"ADMIN_ACTION: {admin_name} updating user {mask_email(email_to_check)}")
        ...
        cache.delete_memoized(get_user_permissions) # Clear auth cache on role change
...
@limiter.limit("20 per minute") # 7.1 Search Rate Limiting
def search_agency():
    username = request.args.get('user', '')
    email = request.args.get('email', '')
    ...
    if not agency_code or not re.match(r'^\d+$', agency_code): 
        return jsonify({"error": "Invalid agency code format"}), 400
...
@limiter.limit("5 per minute") # 7.1 Analytics Rate Limiting
@cache.cached(timeout=300, query_string=True) # 2.1 Caching Analytics Responses
def get_analytics():
    ...
    for page_num in range(150):
        # 2.4 Future optimization: Replace automatic_fields with explicit field_names array if payload size becomes an issue
