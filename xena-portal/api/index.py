...
def clean(field_data):
    """The Trailing Space Destroyer: Prevents 'Done ' bugs and standardizes strings."""
    return extract_field_text(field_data).strip().lower()

def parse_feishu_date(date_val):
    """Safely converts timestamps or strings into pure YYYY-MM-DD objects."""
    ...
    try:
        if isinstance(date_val, (int, float)):
            # Convert timestamp to Egypt time (+3 hours) safely
            return (datetime.fromtimestamp(date_val / 1000.0, timezone.utc).replace(tzinfo=None) + timedelta(hours=3)).replace(hour=0, minute=0, second=0, microsecond=0)
        
        date_str = str(date_val).strip()
        if date_str.isdigit():
            return (datetime.fromtimestamp(int(date_str) / 1000.0, timezone.utc).replace(tzinfo=None) + timedelta(hours=3)).replace(hour=0, minute=0, second=0, microsecond=0)
        
        # Handle string formats like "2026/07/05"
...
def get_analytics():
    ...
    # 1. DEFAULT TO "PK" AND "CURRENT MONTH" IF NOT PROVIDED
    ...
    # 2. PURE FETCH LOOP (NO EARLY EXITS, NO SORTING BUGS)
    req_url = f"https://open.feishu.cn/open-apis/bitable/v1/apps/{BASE_ID}/tables/{REQUESTS_TABLE_ID}/records/search?automatic_fields=true"
    ...
    for _ in range(100):  # 100 pages is enough to pull up to 50,000 rows quickly
        if page_token: payload["page_token"] = page_token
        try:
            res = session.post(req_url, json=payload, timeout=20)
            if res.status_code != 200:
                error_msg = f"HTTP {res.status_code}: {res.text}"
                break
                
            res_json = res.json()
            # If native Region filter is rejected, drop it and fetch everything safely
    ...
    for item in all_items:
        ...
        if record_dt < from_dt: continue
        if record_dt >= to_dt: continue

        # ENFORCE REGION EXACTLY
        ...
        acm = clean(get_field_local(fields, 'Acm Name (PK)', 'Acm Name (IN)', 'Assigned Member'))
        acm_title = acm.title() if acm else "Unknown"
        ...
        # KPI #12 Daily Trend (All records strictly in 'Done' status)
        if is_done:
            date_str = record_dt.strftime("%Y-%m-%d")
            if date_str in stats["daily_trend"]:
                stats["daily_trend"][date_str] += 1

        # 4. KPI ASSIGNMENT (Exact "Contains" logic from video)
        if "closing agency" in req_type:
            ...
            if closing_reason:
                ...
                if acm_title != "Unknown":
                    if acm_title not in stats["acm_closing_reasons"]:
                        stats["acm_closing_reasons"][acm_title] = {"User Request": 0, "Duplicated Hosting": 0}
                    if "user" in closing_reason:
                        stats["acm_closing_reasons"][acm_title]["User Request"] += 1
                    elif "dup" in closing_reason:
                        stats["acm_closing_reasons"][acm_title]["Duplicated Hosting"] += 1
            ...
            if is_done and acm_title != "Unknown":
                stats["acm_performance"][acm_title] = stats["acm_performance"].get(acm_title, 0) + 1
            ...
            creation_type = clean(get_field_local(fields, 'Create Way', 'Agencies Creation Type', 'PK Agencies Creation Type'))
            if creation_type:
