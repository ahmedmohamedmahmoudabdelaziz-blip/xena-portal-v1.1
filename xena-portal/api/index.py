...
# ════════════════════════════════════════════════════════════════════
# CORE UTILITIES, TIMEZONE & LOCAL DATA MANAGEMENT
# ════════════════════════════════════════════════════════════════════
CAIRO_OFFSET = timedelta(hours=3)

# Data Directory for Cron-Fetched JSON
DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "public", "data")

def load_local_json(filename):
    """Loads a pre-fetched JSON file generated by fetch_data.py to bypass Feishu APIs"""
    ...
    if os.path.exists(path):
        try:
            ...
            logger.error("load_json_failed", file=filename, error=str(e))
    return None
...
def get_user_permissions(email, name):
    ...
    # ⚡ Use Local JSON first if available
    access_data = load_local_json("access.json")
...
# ════════════════════════════════════════════════════════════════════
# CORE DATA FETCHING ENGINE (FALLBACK ONLY)
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
# SNAPSHOT MANAGER (Re-wired to load Local JSON)
# ════════════════════════════════════════════════════════════════════

def get_requests_table_snapshot(from_dt=None):
    """Prioritizes local JSON generated by cron for millisecond speed"""
    local_data = load_local_json("requests.json")
    if local_data is not None:
        keys = set(local_data[0].get("fields", {}).keys()) if local_data else set()
        return local_data, keys, True, "", True
...
def get_points_table_snapshot():
    ...
    items, _keys, complete, reason = fetch_points_sharded()
    return items, complete, reason, False


# ════════════════════════════════════════════════════════════════════
# CORE LOGIC: FETCH SINGLE AGENCY (Re-wired to Local JSON)
# ════════════════════════════════════════════════════════════════════
def fetch_agency_data(code, query_type="points", allowed_acms=None, allowed_regs=None):
    if MOCK_MODE:
        ...
        # ⚡ Use Local JSON to completely bypass Feishu `is` operator bug
        local_points = load_local_json("points.json")
        if local_points is not None:
            all_records = []
            for r in local_points:
                f = r.get("fields", {})
                c = extract_field_text(get_field_local(f, "Agency Code")).strip()
                if c == str(code).strip():
    ...
    if MOCK_MODE:
        ...
        # ⚡ Use Local JSON to completely bypass Feishu timeline search bug
        local_reqs = load_local_json("requests.json")
        if local_reqs is not None:
            hist_items = []
            for r in local_reqs:
                f = r.get("fields", {})
                c = extract_field_text(get_field_local(f, "Agency Code")).strip()
...
def manage_users():
    ...
    if request.method == 'GET':
        access_items = load_local_json("access.json")
...
def audit_logs():
    ...
    local_audit = load_local_json("audit.json")
    if local_audit is not None:
        logs = []
        for item in local_audit:
            f = item.get("fields", {})
        ...
        return jsonify(logs[:min(int(request.args.get('limit','100')), 500)])
...
def points_records():
    ...
    # Background Snapshot Priority: hit the Local JSON generated by fetch_data.py
...
def query_records():
    ...
    if MOCK_MODE:
        ...
        # ⚡ Use Local JSON by default
        local_reqs = load_local_json("requests.json")
        if local_reqs is not None:
            all_items = []
            aliases = QUERY_FIELD_ALIASES[field]
            val_lower = str(value).lower()
            for item in local_reqs:
                f = item.get("fields", {})
                match = False
                for alias in aliases:
                    val_in_db = str(extract_field_text(get_field_local(f, alias))).lower()
                    if val_lower in val_in_db:
                        match = True
                        break
                if match:
                    all_items.append(item)
...
def analytics():
    ...
    # Background Snapshot Priority: pull from Local JSON generated by fetch_data.py
