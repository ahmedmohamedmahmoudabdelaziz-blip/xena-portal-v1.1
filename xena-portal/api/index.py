...
Live server‑side filtering on safe fields only.
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# FILTER BUILDERS – SAFE FIELDS ONLY (no ACM, no date)
# ──────────────────────────────────────────────────────────────────────────────
def build_analytics_filter(region, agency_type):
    """
    Build a filter for the Requests table.
    Only Region and Agency Type – ACM and date are filtered in‑memory.
    """
    conditions = []
...
def build_points_filter(agency_code, region, search):
    ...
    Only Agency Code, Region, and search on Agency Code/Name.
...
def run_analytics(all_items, from_dt, to_dt, region_filter, acm_filter, type_filter,
    ...
    for item in all_items:
        ...
        raw_acm_fb = get_field_local(fields, "Assigned Member")
...
# ──────────────────────────────────────────────────────────────────────────────
# AGENCY SEARCH (unchanged – uses safe fields)
# ──────────────────────────────────────────────────────────────────────────────
def fetch_agency_data(code, query_type="points", allowed_acms=None, allowed_regs=None):
    ...
    acm_raw      = extract_field_text(get_field_local(first,"Acm Name (PK)","Acm Name (IN)","Assigned Member"))
...
# ──────────────────────────────────────────────────────────────────────────────
# REFACTORED POINTS RECORDS ENDPOINT – safe fields only
# ──────────────────────────────────────────────────────────────────────────────
...
def points_records():
    ...
    f_acm        = sanitize_text(request.args.get('acm','')).lower()  # in-memory filter
    ...
    if MOCK_MODE:
        ...
        field_names = [
            "Agency Code", "Agency Name", "Region",
            "Acm Name (PK)", "Acm Name (IN)",
    ...
    for item in all_items:
        ...
        acm_pk      = extract_field_text(get_field_local(f, "Acm Name (PK)")).strip()
        acm_in      = extract_field_text(get_field_local(f, "Acm Name (IN)")).strip()
        # fallback to region-based ACM if needed
        region_raw  = extract_field_text(get_field_local(f, "Region")).strip().upper()
        if region_raw in ("PK", "IN"):
            acm = acm_pk if region_raw == "PK" else acm_in
        else:
            # try to determine from PK/IN sets
            if acm_pk.lower() in PK_ACMS: acm = acm_pk
            elif acm_in.lower() in IN_ACMS: acm = acm_in
            else: acm = acm_pk or acm_in  # fallback
        region = 'PK' if acm.lower() in PK_ACMS else ('IN' if acm.lower() in IN_ACMS else region_raw)
...
def query_records():
    ...
    for item in all_items:
        ...
        acm_fb = clean(get_field_local(fields, "Assigned Member"))
        if region in ("", "none"):
...
# ──────────────────────────────────────────────────────────────────────────────
# REFACTORED ANALYTICS ENDPOINT – safe filters (Region & Agency Type only)
# ──────────────────────────────────────────────────────────────────────────────
...
def analytics():
    ...
    # Safe field projection – only fields known to exist
    field_names = [
        "Request Type", "Status", "Region",
        "Acm Name (PK)", "Acm Name (IN)", "Assigned Member",
...
# ──────────────────────────────────────────────────────────────────────────────
# REFACTORED COMPARE ENDPOINT – uses same safe filters
# ──────────────────────────────────────────────────────────────────────────────
...
def compare():
    ...
    for label, from_dt, to_dt, acm_filter in groups_spec:
        if MOCK_MODE:
            ...
            filter_obj = build_analytics_filter(region_filter, type_filter)
            all_items, fetch_complete, _ = fetch_records_with_filter(REQUESTS_TABLE_ID, filter_obj, safe_field_names)
            if not fetch_complete:
                all_items = []
        stats = run_analytics(all_items, from_dt, to_dt, region_filter, acm_filter, type_filter, allowed_acms, allowed_regs)
...
def health():
    return jsonify({
        ...
        "message": "Analytics endpoints use server-side filtering (Region & Agency Type) only."
    })
