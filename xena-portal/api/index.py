...
def fetch_requests_sharded(from_dt=None, to_dt=None, field_names=REQUESTS_ANALYTICS_FIELDS):
    """LIVE replacement for fetch_feishu_records(REQUESTS_TABLE_ID, ...) on the Analytics /
    Compare hot paths. Splits the table into concurrent Region shards (+ a catch-all shard
    for blank Region rows, which run_analytics already knows how to infer from ACM), and
    layers a server-side date-range filter on top when the caller has one. Nothing is cached
    here — every call re-hits Feishu.
    """
    if MOCK_MODE:
        items = MockFeishuDB.generate_requests(300)
        keys = set(items[0]["fields"].keys()) if items else set()
    ...
    date_conditions = []
    if from_dt:
        date_conditions.append({"field_name": "Submitted on Copy", "operator": "isGreater",
                                 "value": _date_filter_value(from_dt - timedelta(days=1))})
    if to_dt:
        date_conditions.append({"field_name": "Submitted on Copy", "operator": "isLess",
                                 "value": _date_filter_value(to_dt)})

    region_shards = [
        {"conjunction": "and", "conditions": [{"field_name": "Region", "operator": "is", "value": [r]}] + date_conditions}
        for r in ("PK", "IN")
    ]
    catch_all_shard = {"conjunction": "and", "conditions": [{"field_name": "Region", "operator": "isEmpty"}] + date_conditions}
    all_shards = region_shards + [catch_all_shard]

    with ThreadPoolExecutor(max_workers=len(all_shards)) as executor:
        futures = [executor.submit(_fetch_bitable_shard, REQUESTS_TABLE_ID, tat, f, field_names) for f in all_shards]
...
def fetch_points_sharded(field_names=POINTS_TABLE_FIELDS):
    ...
    Same Region-shard-in-parallel + field-projection approach as fetch_requests_sharded."""
    if MOCK_MODE:
    ...
    tat = get_tenant_access_token()
    region_shards = [
        {"conjunction": "and", "conditions": [{"field_name": "Region", "operator": "is", "value": [r]}]}
        for r in ("PK", "IN")
    ]
    catch_all_shard = {"conjunction": "and", "conditions": [{"field_name": "Region", "operator": "isEmpty"}]}
    all_shards = region_shards + [catch_all_shard]

    with ThreadPoolExecutor(max_workers=len(all_shards)) as executor:
        futures = [executor.submit(_fetch_bitable_shard, POINTS_TABLE_ID, tat, f, field_names) for f in all_shards]
        shard_results = [f.result() for f in futures]

    all_items, _keys, fetch_complete, stop_reason = _merge_shards(shard_results)
