...
def write_build_meta():
    """Stamp the build's real completion time into a small sidecar file, since
    Vercel's deployment packaging does NOT preserve the actual file write time
    when it bundles public/data/** into the serverless function -- reading it
    back via os.path.getmtime() at runtime returns a fixed, meaningless value
    (observed: Oct 2018) no matter how recently the build actually ran. That
    silently broke the app's "is my on-disk snapshot fresh?" check, which
    always saw the file as years old and never trusted it -- meaning it fell
    through to a live Feishu fetch on every single request, regardless of
    deploy frequency. Writing our own timestamp, generated by this script at
    the moment it actually finished, gives index_4.py something trustworthy
    to read instead of relying on a filesystem attribute the platform resets.
    """
    path = os.path.join(DATA_DIR, "_build_meta.json")
    meta = {
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump(meta, f)
    log(f"🕒 Wrote build meta: generated_at={meta['generated_at_iso']}")
